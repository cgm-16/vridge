...
kubectl create secret generic vridge-env \
  --namespace vridge \
  --from-literal=BETTER_AUTH_SECRET=<값> \
  --from-literal=BETTER_AUTH_URL=<값> \
  --from-literal=NEXT_PUBLIC_APP_URL=<값> \
  --from-literal=NEXT_PUBLIC_GA_MEASUREMENT_ID=<값> \
  --from-literal=NEXT_PUBLIC_PRIVACY_POLICY_URL=<값>
```
...
kubectl create secret generic vridge-env \
  --namespace vridge \
  --from-literal=BETTER_AUTH_SECRET=<새값> \
  --from-literal=BETTER_AUTH_URL=<새값> \
  --from-literal=NEXT_PUBLIC_APP_URL=<새값> \
  --from-literal=NEXT_PUBLIC_GA_MEASUREMENT_ID=<새값> \
  --from-literal=NEXT_PUBLIC_PRIVACY_POLICY_URL=<새값> \
